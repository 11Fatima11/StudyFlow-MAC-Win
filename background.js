...
chrome.runtime.onInstalled.addListener(() => {
  ...
  chrome.contextMenus.create({ id: "canvas-ai-use",       title: "Use in StudyFlow",       contexts: ["selection"] });
...
async function bgExtract(url, filename, kind) {
  ...
  // Detect by extension first, then content-type, then kind hint
...
function extractPptxBlocks(xml, chartFiles) {
  ...
  // Step 4 — plain text paragraphs (strip tables/charts/pics first)
  let textXml = xml
    .replace(/<a:tbl>[\s\S]*?<\/a:tbl>/g, "")
    .replace(/<p:graphicFrame>[\s\S]*?<\/p:graphicFrame>/g, "")
    .replace(/<p:pic[\s>][\s\S]*?<\/p:pic>/g, "");

  const paraRe = /<a:p[\s>][\s\S]*?<\/a:p>/g;
  let pp;
  const textParts = [];
  while ((pp = paraRe.exec(textXml)) !== null) {
    const runs = [];
    const tRe  = /<a:t[^>]*>([\s\S]*?)<\/a:t>/g;
    let tr;
    while ((tr = tRe.exec(pp[0])) !== null) {
...
function extractXmlTable(xml) {
  ...
  return rows.length ? toMarkdownTable(rows) : "";
}
...
function extractChartSeries(xml) {
  ...
  const catM = /<c:cat[\s\S]*?<\/c:cat>/.exec(xml);
  if (catM) {
    const cats = [];
...
async function extractDocx(buf) {
  if (typeof mammoth !== "undefined") {
    try {
      ...
      if (htmlResult.value && htmlResult.value.trim()) return htmlToMarkdown(htmlResult.value);
...
function htmlToMarkdown(html) {
  ...
  md = md.replace(/<img[^>]*alt="([^"]*)"[^>]*>/gi, (_, alt) => alt ? `[Image: ${alt}]` : "[Image]");
...
function extractDocxXml(xml) {
  const parts  = [];
  const bodyM  = /<w:body[\s>]([\s\S]*)<\/w:body>/.exec(xml);
...
async function extractXlsx(buf) {
  ...
  while ((m = snRe.exec(workbookXml)) !== null) sheetNames.push({ name: m[1], rid: m[2] });
  ...
  for (const sheet of sheetNames) {
    ...
    while ((m = cellRe.exec(sheetXml)) !== null) {
      ...
      let   val   = "";
      if (vM) {
        val = t === "s" ? (shared[+vM[1]] ?? vM[1])
            : t === "b" ? (vM[1] === "1" ? "TRUE" : "FALSE")
            : vM[1];
      }
      const isM = inner.match(/<is>[\s\S]*?<t[^>]*>([\s\S]*?)<\/t>/);
      if (isM) val = xmlDec(isM[1]);
...
async function extractPdf(buf) {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    if (maxCols >= 2 && tableRows.length >= 3 && tableRows.length / rowArrays.length > 0.5) {
      pages.push(`[Page ${i}]\n${toMarkdownTable(tableRows.map(r => r.map(c => c.text)))}`);
    } else {
      pages.push(`[Page ${i}]\n${rowArrays.map(r => r.map(c => c.text).join("  ")).join("\n")}`);
    }
  }
